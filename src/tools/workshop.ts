...
function metadata(){const d=readDefinition(session.sim,kind)!;element('asset-name').textContent=names[kind]??String(kind);element('definition').textContent=`${CIVILIZATIONS[d.faction]} · ${d.klass===0?'Building':'Unit'} · ${d.health} HP · ${d.speed.toFixed(2)} speed · ${d.range} range · ${d.damage} damage · ${d.alloy} Alloy / ${d.charge} Charge · ${d.ticks/60}s production`;}
...
function options(){
 ...
 element<HTMLSelectElement>('variant').disabled=kind!==30;roster();metadata();route();
}
function update(){
