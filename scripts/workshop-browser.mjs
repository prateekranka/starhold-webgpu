...
try{
 ...
 for(const [name,width,height] of [['desktop',1440,1000],['phone',390,844],['landscape',844,390],['ipad',768,1024]]){
  ...
  // A software GPU may finish readback after DOM readiness. Require real pixels,
  // rather than considering either a fixed delay or an empty screenshot a pass.
  await page.waitForFunction(()=>{const canvas=document.querySelector('#native'),data=canvas.getContext('2d').getImageData(0,0,canvas.width,canvas.height).data,colors=new Set();for(let i=0;i<data.length;i+=4)colors.add(`${data[i]},${data[i+1]},${data[i+2]}`);return colors.size>3;},null,{timeout:15000});
  ...
  assert.deepEqual(errors,[]);assert.equal(await page.locator('#workshop').getAttribute('data-error'),null,await page.locator('#status').textContent());results.push({viewport:name,pass:true,paintedColors:painted,metrics:state});console.log('PASS',name,JSON.stringify(state));await context.close();activePage=null;
 }
...
}catch(error){results.push({pass:false,error:String(error)});if(activePage){const diagnostics=await activePage.evaluate(()=>({state:window.__WORKSHOP?.getState(),status:document.querySelector('#status')?.textContent,canvases:[...document.querySelectorAll('canvas')].map(c=>({id:c.id,width:c.width,height:c.height,visible:!c.hidden}))})).catch(()=>({}));console.error('DIAGNOSTICS',JSON.stringify(diagnostics));await activePage.screenshot({path:'workshop-evidence/failure.png',fullPage:true}).catch(()=>{});await writeFile('workshop-evidence/failure.txt',await activePage.locator('body').innerText().catch(()=>''));}throw error;}
