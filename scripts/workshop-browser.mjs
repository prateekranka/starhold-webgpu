...
try{
 ...
 for(const [name,width,height] of [['desktop',1440,1000],['phone',390,844],['landscape',844,390],['ipad',768,1024]]){
  ...
  await page.waitForFunction(()=>document.querySelector('#workshop')?.dataset.ready==='true'||document.querySelector('#workshop')?.dataset.error==='true',{timeout:60000});
  assert.equal(await page.locator('#workshop').getAttribute('data-error'),null,await page.locator('#status').textContent());
  await page.waitForTimeout(250);
  assert.equal(await page.locator('#asset-name').textContent(),'Ash Jackal');
  await page.locator('#variant').selectOption('longbow');await page.locator('#state').selectOption('2');
  await page.locator('#phase').fill('0.49');await page.locator('#phase').dispatchEvent('input');
  await page.waitForTimeout(150);await page.screenshot({path:`workshop-evidence/${name}-forge.png`,fullPage:true});
  ...
  await page.locator('button[data-research="202"]').click();
  await page.evaluate(()=>window.__WORKSHOP.step(1200));
  ...
  // The interactive timeline is capped at 30s, so the second job is still in flight.
  assert.ok((await page.locator('[data-node="203"] button').textContent()).includes('remaining'));
  ...
  await page.locator('#doctrine').selectOption('running');await page.locator('#reset').click();
  await page.evaluate(()=>window.__WORKSHOP.step(60));
  ...
  assert.deepEqual(errors,[]);results.push({viewport:name,pass:true,paintedColors:painted,metrics:state});await context.close();
 }
 const page=await browser.newPage();await page.goto('http://127.0.0.1:5199/');
 ...
 await page.evaluate(()=>window.__APP.startMatch(1));await page.locator('#hud-research').click();
 assert.equal(await page.locator('.research-dialog').evaluate(d=>d.open),true);
 await page.locator('.research-close').click();
 results.push({view:'production-match-research',pass:true});
 console.log(JSON.stringify(results,null,2));
}catch(error){results.push({pass:false,error:String(error)});throw error;}
